...
class CodeGeneratorScreen(Screen):
    ...
    def update_text_and_generate(self, text):
        ...
        current_text = self.ids.user_input.text
        if current_text and not current_text.endswith(' '):
            self.ids.user_input.text = f"{current_text} {text}"
        else:
            self.ids.user_input.text = f"{current_text}{text}"
        
        self.ids.feedback_label.text = "Voice input added, generating response..."
        
        # Now automatically generate the response
        # Use a small delay to ensure UI updates before starting the API call
        Clock.schedule_once(lambda dt: self.ask_question(), 0.5)
    ...
    def _update_response(self, response_text):
        ...
        self.ids.feedback_label.text = "Response received successfully!"
