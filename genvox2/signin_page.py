...
class SignInScreen(Screen):
    def __init__(self, **kwargs):
        ...
        form_container.add_widget(sign_in_label)

        # Form Fields (Name, Email, Password)
        form_layout = GridLayout(cols=1, spacing=10, size_hint_y=None, height=200)
        ...
        self.spassword_input = TextInput(
        ...
        form_layout.add_widget(self.spassword_input)
        ...
        login_label.bind(on_ref_press=self.go_to_login)
    ...
    def submit(self, instance):
        """Store new user credentials in MongoDB"""
        name = self.name_input.text.strip()
        email = self.email_input.text.strip()
        password = self.password_input.text.strip()
        confirm_password = self.spassword_input.text.strip()
        ...
        self.display_message(result)  # Show success or error message

        if "successfully" in result:
            # Add a subtle fade-out animation before changing screens
            fade_out = Animation(opacity=0, duration=0.3)
            fade_out.bind(on_complete=lambda *args: self.go_to_login_transition())
            fade_out.start(self)

    def go_to_login_transition(self):
        self.manager.current = "login"  # Navigate to login
    ...
    def go_to_login(self, instance, value):
        """Navigates to Login Page when 'Login' is clicked."""
        fade_out = Animation(opacity=0, duration=0.3)
        fade_out.bind(on_complete=lambda *args: self.go_to_login_transition())
        fade_out.start(self)
    ...
    def go_back(self, instance):
        ...
        fade_out = Animation(opacity=0, duration=0.3)
        fade_out.bind(on_complete=lambda *args: self.set_index_screen())
        fade_out.start(self)
    
    def set_index_screen(self):
        self.manager.current = "index"  # Go to Index Page
        self.opacity = 1  # Reset opacity for next time
